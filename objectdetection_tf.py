    ...
    def inputImage(self):
            ...
            try:
                ...
                for i,x in enumerate(imgExtract[selectedObj]):
                    ...
                    c.imwrite("saved.jpg",im_Arr[x[1]:x[3],x[0]:x[2]] )
                    img=c.imread("saved.jpg")
                    gb=c.GaussianBlur(img,(5,5),0)
                    mb=c.medianBlur(img,5)
                    bf=c.bilateralFilter(img,9,75,75)
                    plt.figure(figsize=(10,5))

                    plt.figure(figsize=(10, 5))
                    plt.subplot(2, 2, 1)
                    plt.imshow(c.cvtColor(img, c.COLOR_BGR2GRAY))
                    plt.title("Original Grayscale Image")
                    plt.axis("off")
                    plt.subplot(2, 2, 2)
                    plt.imshow(c.cvtColor(gb, c.COLOR_BGR2RGB))
                    plt.title("Gaussian Blur")
                    plt.axis("off")
                    plt.subplot(2, 2, 3)
                    plt.imshow(c.cvtColor(mb, c.COLOR_BGR2RGB))
                    plt.title("Median Blur")
                    plt.axis("off")
                    plt.subplot(2, 2, 4)
                    plt.imshow(c.cvtColor(bf, c.COLOR_BGR2RGB))
